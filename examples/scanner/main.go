...
func main() {
	...
	if err != nil {
		log.Fatalf("Could not read source folder through git")
	}
	...
	sp := src.AddProcessor(sources.NewFileReader(readFile, "name", -1))

	delay := p.AddTransform("Delay")
	f1 := delay.AddProcessor(transforms.DelayFunction(100*time.Millisecond), "path1")

	middleNameRemover := p.AddTransform("Middle Names Remover")
	m1 := middleNameRemover.AddProcessor(
	...
	sink := p.AddSink("Sink")
	sink.AddProcessor(sinks.NewFileWriter(writeFile, "name"), "sink")
